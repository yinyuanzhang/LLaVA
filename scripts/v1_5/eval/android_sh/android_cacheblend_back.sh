...
ANSWERS_FILE_WRITE="$SAVE_DIR/answers-android-control-cacheblend-write-${EVAL_TYPE}-${TIMESTAMP}.jsonl"
ANSWERS_FILE_READ="$SAVE_DIR/answers-android-control-cacheblend-read-load-${EVAL_TYPE}-${TIMESTAMP}.jsonl"
...
# --- Write-only 阶段: 构建缓存 ---
echo "--- Step 1: Building cache with cacheblend (write-only) ---"
...
# --- Read-load 阶段: 使用缓存 ---
echo "--- Step 2: Using cache with cacheblend (read-load) ---"
python llava/eval/llava_android_control.py \
    --model-path $MODEL_BASE \
    --eval-file "$EVAL_FILE" \
...
echo "--- cacheblend evaluation completed successfully ---"